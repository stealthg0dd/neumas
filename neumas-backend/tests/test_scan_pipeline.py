...
async def test_upload_scan_success(monkeypatch):
    ...
    monkeypatch.setattr("app.tasks.scan_tasks.process_scan.apply_async", apply_async)
...
@pytest.mark.anyio
async def test_upload_scan_queue_failure_marks_scan_failed(monkeypatch):
    tenant = TenantContext(
        user_id=uuid4(),
        org_id=uuid4(),
        property_id=uuid4(),
    ...
    monkeypatch.setattr("app.tasks.scan_tasks.process_scan.apply_async", MagicMock(side_effect=RuntimeError("broker down")))

    upload = UploadFile(filename="receipt.jpg", file=BytesIO(b"abc123"), headers={"content-type": "image/jpeg"})

    with pytest.raises(Exception, match="Scan queue unavailable"):
        await service.upload_scan(upload, b"abc123", "receipt", tenant)

    assert scans_repo.update.await_count == 2
...
async def test_upload_route_invalid_file_rejected():
    ...
    assert exc_info.value.detail["code"] == "scan_upload_unsupported_type"


@pytest.mark.anyio
async def test_upload_route_pdf_rejected():
    tenant = TenantContext(
        user_id=uuid4(),
        org_id=uuid4(),
        property_id=uuid4(),
    ...
    upload = UploadFile(filename="receipt.pdf", file=BytesIO(b"%PDF-1.4"), headers={"content-type": "application/pdf"})

    with pytest.raises(HTTPException) as exc_info:
        await upload_scan_route(
            request=SimpleNamespace(state=SimpleNamespace(request_id="req-route-pdf")),
            file=upload,
            scan_type="receipt",
            tenant=tenant,
        )
...
@pytest.mark.anyio
async def test_upload_route_oversized_rejected():
    tenant = TenantContext(
        user_id=uuid4(),
        org_id=uuid4(),
        property_id=uuid4(),
    ...
    upload = UploadFile(filename="receipt.jpg", file=BytesIO(payload), headers={"content-type": "image/jpeg"})

    with pytest.raises(HTTPException) as exc_info:
        await upload_scan_route(
            request=SimpleNamespace(state=SimpleNamespace(request_id="req-route-big")),
            file=upload,
            scan_type="receipt",
            tenant=tenant,
        )
    ...
    assert exc_info.value.status_code == 400
    assert exc_info.value.detail["code"] == "scan_upload_too_large"
...
@pytest.mark.anyio
async def test_process_scan_malformed_ocr_payload_fails_invalid_file(monkeypatch):
    from app.tasks.scan_tasks import _process_scan_async
    ...
    assert result["status"] == "failed_invalid_file"
    assert fake.scans[scan_id]["status"] == "failed_invalid_file"
...
@pytest.mark.anyio
async def test_process_scan_retry_skips_inventory_when_already_applied(monkeypatch):
    from app.tasks.scan_tasks import _process_scan_async

    fake = _FakeSupabase(org_id=str(uuid4()))
    scan_id = fake.scan_id
    fake.scans[scan_id]["status"] = "processing"
    fake.scans[scan_id]["processed_results"] = {
        "stage_details": {
    ...
    stage_details = (fake.scans[scan_id].get("processed_results") or {}).get("stage_details") or {}
    assert stage_details.get("inventory", {}).get("status") == "skipped"
